...
hplt(1) = plot(1e3*E_Fermi_array,1e-8*M_Edelstein_zz,...
    'Color',color_plt_zz);
hold on
hplt(2) = plot(1e3*E_Fermi_array,1e-8*M_Edelstein_xx,...
    'Color',color_plt_xx);
...
%NMR_exp = yline(1e-8*1.3,'--','NMR experiment');
xNMR = linspace(-27,-14,100);
NMR_exp = plot(xNMR,1e-8*1.3 + 0*xNMR,'--');
NMR_exp.Color = [.80 0 .40];
%NMR_exp.LabelHorizontalAlignment = 'center';
%NMR_exp.LabelVerticalAlignment = 'middle';
text(0.1,0.75,'NMR experiment ',...
    'Units','normalized','Color',[.80 0 .40]);
hold off

%grid on
yscale log

yl1 = ylabel('$$M, \mu_{\rm B}$$ per Te', ...
    'Interpreter', 'latex');
pos_yl1 = get(yl1,'Pos');

%ylim([0.5,1.5])
%ylim([0.5,2.0])
%ylim([1e-8*0.5,1e-8*2.0])
%text(0.45,0.75,'$$ M_{z}$$ ',...
text(0.7,0.65,'$$ M_{z}$$ ',...
    'Interpreter', 'latex', ...
...
for isbplt = 1:6
    hplt(isbplt).LineWidth = 1.6;
end

subplot(3,1,1)
text(0.55,0.25,'$$j_{x,z}=100 \, {\rm A} \, {\rm cm}^{-2}$$ ',...
    'Interpreter', 'latex',...
    'Units','normalized');
...
filename_to_save = ...
    'Figures_for_paper4/Observables_vs_EF_for_paper';

...
% test 1:
% if min(abs(velocity(:)))<1e-14 
%     disp(['Hello test 1 =',num2str(min(abs(velocity(:))))]);
%     %velocity(abs(velocity)<1e-14)
% end
...
%DeltaKronEfunc = @(X) E_width^2/(E_width^2 + X^2);
%DeltaKronEfunc = @(X) exp(-(X/E_width)^2)/(E_width*sqrt(pi));
